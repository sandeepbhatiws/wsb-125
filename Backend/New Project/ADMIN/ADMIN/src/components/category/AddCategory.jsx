...
export default function AddCategory() {
    ...
    useEffect(()=>{
        if(params.id!=null){
            axios.post(`http://localhost:5555/api/admin/categories/detail/${params.id}`)
            .then((response)=>{
    ...
    let formhandler = (event)=>{
        ...
        if(params.id!=null){
            axios.put(`http://localhost:5555/api/admin/categories/update/${params.id}`, event.target )
            .then((response)=>{
                        if(response.data.status==true){
            ...
            axios.post('http://localhost:5555/api/admin/categories/add',  event.target )
            .then((response)=>{
                        if(response.data.status==true){
                            // alert(response.data.message)
                            event.target.reset();
